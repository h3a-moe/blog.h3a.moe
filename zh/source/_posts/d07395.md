...
Websocket 已经可以在 HTTP 端口上工作，无需额外分配端口用于 [即时通知](https://github.com/dani-garcia/vaultwarden/wiki/Enabling-WebSocket-notifications) ，该功能现已默认启用。
...
现在甚至有 argon2 hash, 不需要明文 token 了。看看官方教你 [怎么换成 hash](https://github.com/dani-garcia/vaultwarden/wiki/Enabling-admin-page#secure-the-admin_token) .

TL;DR: 使用 bitwarden 缺省设置，同时将 `$` escape 掉，省得 compose 不认。
...
如果您不希望除 CDN 以外的任何访问者知晓您的服务器的真实 IP 地址，可以参考这篇 [隐藏源站 IP](https://kazamasion.medium.com/methods-to-prevent-leaking-websites-origin-server-ip-behind-cdn-2dbcf1dd444c) 的教程。（TL;DR: 使用 IP 白名单和 authenticated pull）
