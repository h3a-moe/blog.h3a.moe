...
但就算贝雕难也有办法解决。可以给 vouch 容器单独开一个 bridge 网络，然后让 nginx 监听网关，就不需要在公网上绕路了。对于部署在同一台机器上的 CI/CD Runner 也是一样的。
...
此外，用 `extra_hosts` 把 gitea 实例的 hostname 解析到网关（主机）上的 nginx 监听的 IP 地址。
...
对于编译安装，我就直接放在默认工作目录 `/opt/vouch-proxy/config/config.yml` , 但是命令行可以指定配置文件路径。
